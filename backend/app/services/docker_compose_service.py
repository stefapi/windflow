...
from app.services.docker_executor import ComposeExecutor
...
class DockerComposeService:
        ...
        try:
            ...
            for service_name, service_config in compose_data["services"].items():
                ...
                if "image" not in service_config and "build" not in service_config:
                    return (
                        False,
                        f"Service {service_name} doit avoir 'image' ou 'build'",
                    )
    ...
    def generate_compose_file(
        self, compose_data: Dict[str, Any], output_path: Path
    ) -> Path:
        """
    ...
    async def deploy_compose(
        ...
        """
        try:
            return await self.executor.deploy_compose(
                compose_file, project_name, env_vars
            )
        except Exception as e:
    ...
    def _guess_variable_type(self, var_name: str, default_value: Optional[str]) -> str:
        ...
        # Password/secret
        if (
            "password" in var_name_lower
            or "secret" in var_name_lower
            or "key" in var_name_lower
        ):
            return "password"
        ...
        if any(
            ...
            for word in ["port", "count", "size", "limit", "workers", "replicas"]
        ):
            return "number"
