...
class CLIDockerExecutor(DockerExecutorBase):
    ...
    async def start_container(self, container_name: str) -> Tuple[bool, str]:
        ...
        try:
            process = await asyncio.create_subprocess_exec(
                self.docker_path, 'start', container_name,
                stdout=asyncio.subprocess.PIPE,
                stderr=asyncio.subprocess.PIPE
            )
    ...
    async def stop_container(
        ...
        try:
            process = await asyncio.create_subprocess_exec(
                self.docker_path, 'stop', '-t', str(timeout), container_name,
                stdout=asyncio.subprocess.PIPE,
                stderr=asyncio.subprocess.PIPE
            )
    ...
    async def restart_container(
        ...
        try:
            process = await asyncio.create_subprocess_exec(
                self.docker_path, 'restart', '-t', str(timeout), container_name,
                stdout=asyncio.subprocess.PIPE,
                stderr=asyncio.subprocess.PIPE
            )
    ...
    async def get_container_status(
        ...
        try:
            process = await asyncio.create_subprocess_exec(
                self.docker_path, 'inspect', container_name,
                stdout=asyncio.subprocess.PIPE,
                stderr=asyncio.subprocess.PIPE
            )
...
class SocketDockerExecutor(DockerExecutorBase):
    ...
    async def _get_client(self) -> Optional[DockerClientService]:
        ...
        if self._client is None:
            try:
                self._client = get_docker_client(socket_path=self.socket_path)
                # Test de connexion
    ...
    async def deploy_container(
        ...
        try:
            ...
            container_config = {
                ...
                "ExposedPorts": {},
                "HostConfig": {
                    "RestartPolicy": {"Name": config.get('restart_policy', 'unless-stopped')},
                    "PortBindings": {},
            ...
            for port_mapping in config.get('ports', []):
                ...
                proto = container_port_proto[1] if len(container_port_proto) > 1 else 'tcp'

                container_config["ExposedPorts"].update({f"{container_port}/{proto}": {}})
            ...
            container_id = await client.create_container(container_config)
    ...
    async def remove_container(
        ...
        try:
            await client.remove_container(container_name, force=force, v=remove_volumes)
...
class DockerExecutor:
    ...
    def __init__(
        ...
        self._cli_available = None  # Lazy check
