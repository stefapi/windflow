...
class StackCreate(StackBase):
    ...
    **Règles de validation:**
    ...
    screenshots: Optional[List[str]] = Field(
        ...
        json_schema_extra={
            "example": ["https://cdn.windflow.io/screenshots/stack-1.png"]
        },
    )
...
class StackUpdate(BaseModel):
    ...
    is_public: Optional[bool] = Field(
        None, description="Statut public", json_schema_extra={"example": True}
    )
    deployment_name: Optional[str] = Field(
        None,
        max_length=255,
...
class StackResponse(StackBase):
    ...
    rating: float = Field(
        default=0.0, description="Note moyenne", json_schema_extra={"example": 4.5}
    )
...
class StackSummaryResponse(BaseModel):
    ...
    version: str = Field(
        ..., description="Version", json_schema_extra={"example": "1.0.0"}
    )
    category: Optional[str] = Field(
        None, description="Catégorie", json_schema_extra={"example": "web"}
    )
    tags: List[str] = Field(
        default_factory=list,
        description="Tags",
    ...
    author: Optional[str] = Field(
        None, description="Auteur", json_schema_extra={"example": "WindFlow Team"}
    )
    license: Optional[str] = Field(
        None, description="Licence", json_schema_extra={"example": "MIT"}
    )
    downloads: int = Field(
