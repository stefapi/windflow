...
class ContainerResponse(BaseModel):
    ...
    ports: list[dict[str, Any]] = Field(default_factory=list, description="Ports exposés")
    labels: dict[str, str] = Field(default_factory=dict, description="Labels du container")
...
class ContainerCreateRequest(BaseModel):
    ...
    name: str = Field(..., description="Nom du container", pattern=r"^[a-zA-Z0-9][a-zA-Z0-9_.-]*$")
    ...
    ports: Optional[dict[str, dict[str, str]]] = Field(None, description="Ports à exposer")
...
class ImageResponse(BaseModel):
    ...
    repo_tags: list[str] = Field(default_factory=list, alias="repoTags", description="Tags")
    repo_digests: list[str] = Field(default_factory=list, alias="repoDigests", description="Digests")
    created: datetime = Field(..., description="Date de création")
...
class VolumeCreateRequest(BaseModel):
    ...
    name: str = Field(..., description="Nom du volume", pattern=r"^[a-zA-Z0-9][a-zA-Z0-9_.-]*$")
    driver: str = Field("local", description="Driver de stockage")
    labels: Optional[dict[str, str]] = Field(None, description="Labels")
...
class SystemInfoResponse(BaseModel):
    ...
    server_version: str = Field(..., alias="ServerVersion", description="Version Docker")
    containers: int = Field(0, description="Nombre total de containers")
    containers_running: int = Field(0, alias="ContainersRunning", description="Containers running")
    containers_paused: int = Field(0, alias="ContainersPaused", description="Containers pausés")
    containers_stopped: int = Field(0, alias="ContainersStopped", description="Containers stoppés")
    images: int = Field(0, description="Nombre d'images")
    driver: str = Field("", description="Driver de stockage")
    docker_root_dir: str = Field("", alias="DockerRootDir", description="Répertoire Docker")
    kernel_version: str = Field("", alias="KernelVersion", description="Version du kernel")
...
class SystemVersionResponse(BaseModel):
    ...
    min_api_version: str = Field("", alias="MinAPIVersion", description="Version API minimale")
...
class ContainerStatsResponse(BaseModel):
    ...
    @classmethod
    def from_docker_stats(cls, container_id: str, data: dict[str, Any]) -> "ContainerStatsResponse":
        return cls(
