...
class TargetBase(BaseModel):
    ...
    host: str = Field(
        ..., min_length=1, max_length=255, description="Hôte (IP ou domaine)"
    )
...
class TargetCreate(TargetBase):
    ...
    credentials: Dict[str, Any] = Field(
        ..., description="Credentials de connexion (seront chiffrés)"
    )
...
class TargetUpdate(BaseModel):
    ...
    name: Optional[str] = Field(
        None, min_length=1, max_length=255, description="Nouveau nom"
    )
    description: Optional[str] = Field(
        None, max_length=500, description="Nouvelle description"
    )
    host: Optional[str] = Field(
        None, min_length=1, max_length=255, description="Nouvel hôte"
    )
    ...
    credentials: Optional[Dict[str, Any]] = Field(
        None, description="Nouveaux credentials"
    )
    status: Optional[TargetStatus] = Field(None, description="Nouveau statut")
    extra_metadata: Optional[Dict[str, Any]] = Field(
        None, description="Nouvelles métadonnées"
    )
...
class TargetResponse(TargetBase):
    ...
    scan_date: Optional[datetime] = Field(
        default=None, description="Date du dernier scan des capacités"
    )
    scan_success: Optional[bool] = Field(
        default=None, description="Indique si le dernier scan des capacités a réussi"
    )
    ...
    extra_metadata: Dict[str, Any] = Field(
        default_factory=dict, description="Métadonnées"
    )
    organization_id: str = Field(..., description="ID de l'organisation")
